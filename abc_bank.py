...
#################################################################################################
# Do sth so as to store customers phone number in the columns of 'ABCBank_Customers' file. DONE!#
# solve customers with same last name issue. DONE!                                              #
#################################################################################################
...
class ABCBank():
    ...
    i = mainPath.index('Bank') + 1
    mainPath = '/'.join(mainPath[:i])+'/'
    ...
    folder = mainPath +'UsersTransactions/'   #path to all customers transactions.

    # This path of the code assumes that 'ABCBank_Customers.csv' already exists in the desired directory
    try:
        file = open(mainPath + 'ABCBank_Customers.csv')  #'ABCBank.csv' is the filename that stores the information of newly added customers
    ...
    def addUser(self, firstName, lastName, otherName='None', phoneNumber='None', accountType='Current', currency='USD', tmpDeposit='5'):
        ...
        self.accountNumbersList = [random.choices(self.numbersList, k=13) for _ in range(15000)] #create 15,000 lists of 13 digits numbers
        ...
        try:
            file = open(self.mainPath + 'ABCBank_Customers.csv')
            lastLine = file.readlines()[-1].strip()
            lastLine = lastLine.split(',')
            self.customerNumber = str(int(lastLine[1])+1)
            self.path = self.lastName + self.customerNumber +'.csv'
        except IOError:
            file = open(self.mainPath + 'ABCBank_Customers.csv', 'a')
        ...
        self.transact(self.accountNumber, 'Deposit', amount=tmpDeposit, reference='ABC Bank',reason='Account Fee')

        return self.accountNumber, self.customerNumber
    ...
    def initializeCustomerAccountNumber(self):
        ...
        folder = self.mainPath + 'EmptyBankAccountsNumber/'  #path to all available unused account numbers
    ...
    def transact(self, accountNumber, transactionType, amount=0, toAccountNumber='N/A', reference='None', reason='None'):
        ...
        *accountNumber
            account Number
        *amount
            amount of money to deposit, withdraw, transfer, my transactions, or my current balance
        *transactionType
        ...
        elif transactionType.lower() == 'my transactions':
            try:
                days = input('Enter last number of days transactions ')
                days = abs(int(days))
            except ValueError:
                while not days.isdigit():
                    print('Please number of days can only be a positive number')
                    days = input('Enter last number of days transactions ')
                days = abs(int(days))
            else:
                self.getUserTransactions(accountNumber, days) #calls the method to get the data for user's previous transactions.
        elif transactionType.lower() == 'current balance':
            customerNumber = self.users[accountNumber]['Customer Number']
            self.checkBalance(customerNumber) #checks balance of customer.
    ...
    def getUserTransactions(self, accountNumber, num=30):
        ...
        folder = self.mainPath + 'UsersTransactions/'
    ...
    def userTransactions(self, date, transactionType, amount, accountNumber, toAccountNumber, reference, reason):
        ...
        folderName = self.mainPath + 'UsersTransactions/'
    ...
    def getReceiptNumber(self):
        filePath = self.mainPath + "receiptNumber.txt"
        try:
            file = open(filePath)  
        except IOError:
            file = open(filePath, "w")
            ...
            tmp_file = open(self.mainPath + "receiptNumber.tmp", "w")
            number = int(file.readline()) + 1
            tmp_file.write(str(number))
            tmp_file.close()
            file.close()
            os.remove(self.mainPath + "receiptNumber.txt")
            os.rename(self.mainPath + "receiptNumber.tmp", self.mainPath + "receiptNumber.txt")
            return number
